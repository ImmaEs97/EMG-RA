"""
Collect raw EMG samples from a Myo armband and save them to CSV.

The script starts a background worker that connects to the Myo, stores each
8-channel EMG sample in memory, periodically writes the partial CSV, and saves
a final CSV when the user presses ENTER or interrupts the program.

Before running:
- change base_dir if you want to save the CSV somewhere other than data/raw;
- keep the Myo close and paired before starting the script;
- use the LED colors as a quick state check: green = connected, blue = running,
  yellow = finished.
"""
...
# ---------------------------------------------------------------------------
# Worker: Myo connection, live collection, and CSV saving
# ---------------------------------------------------------------------------

def data_worker(mode, stop_event, filepath):
    """Collect EMG samples until stop_event is set, then save the final CSV."""
    m = None
    myo_data = []
    ...
    try:
        ...
        # Main acquisition loop: keep polling the Myo until the main thread stops it.
...
# ---------------------------------------------------------------------------
# Main program: output path setup and clean stop from keyboard
# ---------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    # Start a thread instead of a process so the stop_event can be shared directly.
    ...
    try:
        input()  # Press Enter for a clean stop.
