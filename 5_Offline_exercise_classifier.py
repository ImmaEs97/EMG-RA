...
class ExerciseClassifier:
    ...
    def load_data(self):
        ...
        def get_sampling_rate(patient_folder):
            ...
            info_file = patient_folder / "Day1_Patient1_processing_info.txt"
            if not info_file.exists():
                for file in patient_folder.glob("*_processing_info.txt"):
                    info_file = file
                    break
            if info_file.exists():
                try:
            ...
            for ex_file in exercise_files:
                ...
                try:
                    ...
                    if emg_data.shape[0] < patient_window_samples or emg_data.shape[1] != 8:
                        skipped_files.append(f"{ex_file.name} (dati insufficienti: {emg_data.shape})")
                        continue
                    # --- DYNAMIC TRIM ---
        ...
        if ratio < 5:
            print("⚠️  Warning: the number of features is high relative to the number of samples. Risk of overfitting!")
        ...
        if empty_patients:
            print(f"\n⚠️  Participants without valid data: {', '.join(empty_patients)}")
        print(f"{'='*60}\n")
    ...
    def cross_validate_loso(self, shuffle_groups=False, random_state=42, model_to_use=None):
        ...
        print(f"  Accuracy media: {mean_acc:.3f} ± {std_acc:.3f}")
        print(f"{'='*60}")
...
def main():
    ...
    data_dir = Path(__file__).parent / "data" / "Segment_v2"
